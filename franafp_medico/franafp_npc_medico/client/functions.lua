...
TryToGetMedicalTreatment = function(medicIndex)
    ...
    Heap.ESX.TriggerServerCallback("chames_ambulance_medic:requiredAmount", function(required)
        if required then
            ...
            Heap.ESX.ShowNotification("Actualmente hay Médicos en servicio, hable con ellos primero, por favor.")
        end
    end)
end
...
GetMedicalTreatment = function(medicIndex)
    ...
    if not medic.Sequence then return Heap.ESX.ShowNotification("No hay una ruta disponible para el Médico, comuníquese con el personal o el desarrollador.") end
    if not medic.Beds then return Heap.ESX.ShowNotification("No hay camas disponibles, comuníquese con el personal o el desarrollador.") end
...
BedThread = function(medicIndex, bed, bedHandle)
    ...
    --Aquí agregaría todos los eventos que eliminarían el daño si tiene algún recurso personalizado que lo maneje.

    Heap.ESX.ShowNotification("Ha sido tratado, regrese en cualquier momento si necesita ayuda.")
end
